...
#'
#' @export
calculate_interaction_score <- function(expr,ct_map, pairs, n_iterations = 100){ #for spatial data
...
#' @param input SOCIAL object outputted from cis_rslurm() or calculate_interaction_score().
...
#'
#' @export
infer_activity <- function(input, p_cutoff=0.05, median_cutoff=T){
...
#'
#' @export
cis_rslurm <- function(expr,ct_map, pairs, n_iterations = 100){
...
run_cis_rslurm <- function(run, samples, expr_path, ct_map_path, pairs_path, n_iterations){
  ...
  SOCIAL_res = calculate_interaction_score(expr_filter,ct_map, pairs, n_iterations)
...
#'@param n number of pseudopatients/samples for each group
...
#'
#'@export
generate_pseudosamples <- function(n, ct_info, prop=0.4){
